...
describe("Game Logic Tests", () => {
  ...
  describe("getCellBorderStyle", () => {
    ...
    it("should return thick borders between different areas", () => {
      ...
      expect(style.borderBottom).toBe("3px solid black"); // Nedre grænse skal være tyk, da (2,1) er i område C

      style = getCellBorderStyle(2, 2);
      expect(style.borderRight).toBe("3px solid black"); // Højre grænse skal være tyk, da (2,3) er i område B

      style = getCellBorderStyle(4, 1);
      expect(style.borderLeft).toBe("3px solid black"); // Venstre grænse skal være tyk, da (4,0) er i område C

      style = getCellBorderStyle(3, 2);
      expect(style.borderTop).toBe("3px solid black"); // Øverste grænse skal være tyk, da (2,2) er i område D
    });
  });
});
